...
class ParseSAM:
	...
	def samTowig(self,outfile=None,log2scale=False,header=False,strandSpecific=False):
		...
		for line in self.f:
			...
			blocks = cigar.fetch_exon(chrom,txStart,fields[5])	
			for block in blocks:
...
class QCSAM:
	...
	def configure_experiment(self,refbed,sample_size = 200000):
		...
		for line in self.f:
			...
			readEnd = readStart + len(fields[9])
			if (flagCode & 0x0001) != 0:	#paired mapping
				if (flagCode & 0x0040) != 0:	#first read
					read_id = '1'
				if (flagCode & 0x0080) != 0:	#second read
					read_id = '2'
				if (flagCode & 0x0010) == 0:
					map_strand = '+'
				if (flagCode & 0x0010) != 0:
		...
		print "\n\n==================== report ================"
		print "Sequencing = " + protocol		
		if float(specificity) >=90.0:
			print "strand specific: " + "Yes"
			print "strand specificity = " + str(specificity) + '%'
			print "strand plan = " + strandness
		elif float(specificity) >=80.0:
			print "strand specific: " + "Very likely"
			print "strand specificity = " + str(specificity)+ '%'	
			print "strand plan = " + strandness
		elif float(specificity) >=70.0:
			print "strand specific: " + "Likely"
			print "strand specificity = " + str(specificity)+ '%'	
			print "strand plan = " + strandness
		elif float(specificity) >=60.0:
			print "strand specific: " + "Possibly"
			print "strand specificity = " + str(specificity)+ '%'
		else:
			print "strand specific: " + "No"
			print "strand specificity = " + str(specificity)+ '%'
		print "==================== report ================\n"
