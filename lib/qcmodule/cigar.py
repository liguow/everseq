...
def fetch_head_clip(chr, st, cigar):
	...
	NOTE: returned coordinates are 0-based '''
	
	block=[]
	chrom_end = int(st)-1		#because SAM is 1-based
...
def fetch_tail_clip(chr, st, cigar):
	...
	#print read_length
	chrom_end = int(st)-1 + (ref_length - h_len)		#because SAM is 1-based
...
def fetch_insertion(chr, st, cigar):
	...
	NOTE: returned coordinates are 0-based. Insertion to the reference'''
	...
	if len(m)==0:
		...
		for j in m:
			...
			chrom_st = (int(st) -1) + (ref_length - h_len)
...
def fetch_deletion(chr, st, cigar):
	'''return genome coordinates of the insertion (to reference) encoded in cigar string
	NOTE: returned coordinates are 0-based. Deletion to the reference '''
